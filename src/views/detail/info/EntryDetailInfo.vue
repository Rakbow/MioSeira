...
<template>
  <ul>
    <li v-if="entry.nameZh">
      <span>{{ t('NameZh') }}:&nbsp;</span>{{ entry!.nameZh }}
    </li>
    ...
    <li v-if="entry.aliases.length" style="border-bottom: 0">
      <ul>
        <li v-for="(alias, index) in entry.aliases" :class="{ sub: index > 0 }">
          <span v-if="index === 0">{{ t('Aliases') }}:&nbsp;</span>
          {{ alias }}
        </li>
      </ul>
    </li>
