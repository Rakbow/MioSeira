...
onMounted(async () => {
  try {
    ...
    // title.value = album.value ? album.value.name : 'default'; // 设置页面标题
  } catch (error) {
    console.log(error);
    await router.push('/error'); // 跳转到错误页面
  }
});

const getAlbumDetailData = () => {
  console.log(route.params.id)
  let json = {
    id: route.params.id
  };
  AxiosHelper.post(toast, 'http://localhost:8083/db/album/get-album-detail', json)
      .then(res => {
        if (res.state === 1) {
