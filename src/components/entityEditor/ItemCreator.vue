...
const submit = async () => {
  ...
  const res = await axios.post(API.ITEM_CREATE, item.value);
...
<template>
  <BlockUI :blocked="editBlock" class="form-container">
    <SelectButton size="small" v-model="itemType" :options="META.ITEM_TYPE_SET" @change="switchItemType()"
      ...
      <div class="formgrid grid">
        <div class="field col">
          <FloatLabel variant="on">
            ...
            <MultiSelect showClear size="small" v-model="item.albumFormat" :options="store.options.albumFormatSet"
